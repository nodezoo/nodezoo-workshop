...
function dorequest(q,url,cb){
  url = url + '&fields='+['name','desc','latest','maints','site','git','nr','created','modified','git_star','git_fork'].join(',')
  //console.log(url)

  request( url, function( err, res, body ){
    if( err ) {
      log.err('search', '~'+q+'~'+err)
      return cb(err);
    }
    ...
    if( hits ) {
      ...
      for( var i = 0; i < hits.length; i++ ) {
        ...
        //delete hit.readme; console.dir(hit);

        var row = hit._source ? hit._source : hit.fields ? hit.fields : hit
        items.push({
          name:row.name,
          desc:row.desc,
          latest:row.latest,
...
function insertall(args,cb) {
  ...
  var rankfile = args.rankfile
  var npmfile  = args.npmfile
...
function nodezoo(opts,cb) {
  ...
  si.add({role:'nodezoo',cmd:'getrepo'},function(args,cb){
    ...
    github.repos.get(
      ...
      function(err,repo){
        if( err ) {
	  console.dir(err)
          console.log(new Date().toISOString()+': GETREPO '+err)
          if( ~err.message.indexOf("Not Found") ) {
            console.log('sending empty repo')
            return cb(null,{})
          }
          else return cb(err);
        }
        console.log( new Date().toISOString()+': '+JSON.stringify((repo.meta?repo.meta:{})) )
  ...
  si.add(
    ...
    function(args,cb){
      ...
      console.log('POST '+ new Date().toISOString()+': '+url)
      request(
        {
          method:'POST',
          url:url,
        ...
        function(err,res,bdy){
          console.log('UPDATED: '+args.name+' '+err)
          if( err ) {
            console.dir(bdy)
          }
          cb(err,{ok:!err,name:args.name})
        })

      var modent = si.make('mod')
      var q = {name:args.name}
      modent.load$(q,function(err,mod){
        if(err) {
	...
	//console.log(JSON.stringify(q)+': '+mod)
        mod.save$()
      })
    })
  ...
  si.act({role:'config',cmd:'get',base:'nodezoo',default$:{}},function(err,config){
    ...
    opts = _.extend({
      search: {
        hosturl:"http://nodezoo.com",
        query: {
          prefix:'/api/query?q=',
          xprefix:"/zoo/doc/_search?q=",
          suffix:"&timeout=1111&size=22",
        },
      }},opts,config)
